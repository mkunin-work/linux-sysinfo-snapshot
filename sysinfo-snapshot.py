...
def add_external_file_if_exists(field_name, curr_path):
    ...
    if (field_name.startswith("/var/log")):
        ...
        if is_command_allowed("ps -eLo"):
            status, command_output = get_status_output("ps -eLo lstart,%cpu,psr,nlwp,f,tid,uid,pid,ppid,pri,rtprio,ni,vsz,rss,stat,tty,time,wchan,args")
            if (status == 0):
                add_ext_file_handler(field_name, field_name, command_output)
            else:
                err_flag = 1
                err_command += "ps -eLo lstart,%cpu,psr,nlwp,f,tid,uid,pid,ppid,pri,rtprio,ni,vsz,rss,stat,tty,time,wchan,args"
